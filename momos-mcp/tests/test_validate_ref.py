...
import momos_mcp._git as server
...
def test_valid_ref_returns_none():
    with patch("momos_mcp._git.subprocess.run", return_value=_mock_run(0)):
        assert server._validate_ref("main") is None


def test_invalid_ref_returns_error_string():
    with patch("momos_mcp._git.subprocess.run", return_value=_mock_run(1)):
...
def test_injection_attempt_passed_as_list_arg():
    ...
    with patch("momos_mcp._git.subprocess.run", side_effect=fake_run):
